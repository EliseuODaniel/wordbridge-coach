...
6. Include a runtime smoke check when the change touches frontend/API boundaries, compose, startup config, or profile behavior.
7. Record architectural or workflow-impacting changes in `docs/DECISIONS.md`, `docs/ROADMAP.md`, or `docs/CODEX_SETUP.md` when needed.
...
- Treat source-mode, Vite-mode, and container-mode behavior as separate contracts until all three are validated.
- User-facing actions must not fail silently; expose a clear error state and keep the console detail for diagnosis.
